...
void slow_thread() {
    while (Cache::running) {
        ...
        for (int i = 0; i < entityCount; i++) {
            ...
            PlayerData data;
            data.address = entity;
            data.visualState = bp.GetVisualState();
            data.name = bp.GetName();
            data.isSleeping = bp.IsSleeping();
            data.isValid = true;

            uintptr_t boneList = bp.GetBoneTransforms();
            if (boneList) {
                for (int b = 0; b < 17; b++) {
                    data.boneTransforms[b] = bp.GetBoneTransform(boneList, boneIds[b]);
                }
            }
    ...
    while (Cache::running) {
...

        {
            ...
            for (auto& player : Cache::players) {
                if (!player.visualState) continue;
                BasePlayer bp(player.address);
                player.position = bp.GetPosition(player.visualState);

                for (int b = 0; b < 17; b++) {
                    if (player.boneTransforms[b]) {
                        player.bonePositions[b] = bp.GetPosition(player.boneTransforms[b]);
                    }
                }
            }
        }
