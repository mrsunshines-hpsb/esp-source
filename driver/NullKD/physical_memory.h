...
/* ── Batch Physical Read (CMD_READ_BATCH) ────────────────────────────── */
/*
 * Reads multiple virtual addresses from the target process in ONE kernel call.
 * One CR3 lookup, one pool allocation, N page-table walks — vs N kernel round-trips.
 * Each entry reads up to 64 bytes into batchReq->outBuffer at the given outOffset.
 * Entries that fail to translate are silently skipped (output bytes left zero).
 */
static BOOL PhysicalBatchReadProcessMemory(
    HANDLE pid, PBATCH_READ_REQUEST batchReq, SIZE_T outBufSize)
{
    if (!batchReq || batchReq->count == 0 || batchReq->count > BATCH_MAX_ENTRIES)
        return FALSE;
    if (!batchReq->outBuffer || !outBufSize)
        return FALSE;

    ULONG64 cr3 = GetProcessCR3(pid);
    if (!cr3) return FALSE;

    /* Allocate one kernel staging buffer large enough for all results */
    PUCHAR kernelOut = (PUCHAR)ExAllocatePoolWithTag(NonPagedPool, outBufSize, 'bPhM');
    if (!kernelOut) return FALSE;
    RtlZeroMemory(kernelOut, outBufSize);

    /* Small per-read staging buffer (max 64 bytes per entry) */
    UCHAR entryBuf[64] = { 0 };

    for (ULONG i = 0; i < batchReq->count; i++) {
        PBATCH_READ_ENTRY e = &batchReq->entries[i];
        if (!e->address || e->size == 0 || e->size > 64) continue;
        if (e->outOffset + e->size > (ULONG)outBufSize) continue;

        RtlZeroMemory(entryBuf, e->size);

        /* Page-table walk for this address */
        ULONG64 physAddr = TranslateVirtualAddress(cr3, e->address);
        if (!physAddr) continue;

        /* Read from physical address into staging buffer */
        if (!NT_SUCCESS(ReadPhysicalAddress(physAddr, entryBuf, e->size))) continue;

        /* Copy result into the output buffer at the specified offset */
        RtlCopyMemory(kernelOut + e->outOffset, entryBuf, e->size);
    }

    /* Copy all results to usermode output buffer */
    BOOL success = FALSE;
    __try {
        RtlCopyMemory((PVOID)batchReq->outBuffer, kernelOut, outBufSize);
        success = TRUE;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        success = FALSE;
    }

    ExFreePoolWithTag(kernelOut, 'bPhM');
    return success;
}
